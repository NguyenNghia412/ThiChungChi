...
on b.FacultyID = c.ID where a.ID in (select StudentID from[dbo].[AS_Academy_Student_SV_XacNhanUuDaiTrongGiaoDuc] where ID = {0}); ", ID);
            DataSet ds = Microsoft.ApplicationBlocks.Data.SqlHelper.ExecuteDataset(Nuce_Common.ConnectionString, CommandType.Text, sql);
            ...
            if (dt1.Rows.Count > 0 && dt2.Rows.Count > 0)
            {
                ...
                int commonFontSize = 13;
                #region Mẫu số
                Paragraph paragraphMauSo = new Paragraph(
                    document,
                    new Run(document, "Mẫu số 02/ƯĐGD")
                    {
                        CharacterFormat = new CharacterFormat { Bold = true, Size = commonFontSize }
                    })
                {
                ...
                new SpecialCharacter(document, SpecialCharacterType.LineBreak),
                new Run(document, string.Format("{0}", "---------------"))
                {
                    CharacterFormat = new CharacterFormat()
                    {
                        Size = 13
               ...
               , new SpecialCharacter(document, SpecialCharacterType.LineBreak)
                , new Run(document, "Hiện là sinh viên: ")
                {
                    CharacterFormat = new CharacterFormat()
                    {
                        Size = commonFontSize
                ...
                , new SpecialCharacter(document, SpecialCharacterType.Tab), new SpecialCharacter(document, SpecialCharacterType.Tab)
               ...
               , new Run(document, "Khoa/Ban: ")
               {
                ...
                #endregion
                #region Khoá học
                Table tableKhoaHoc = new Table(document);
                tableKhoaHoc.TableFormat.PreferredWidth = new TableWidth(100, TableWidthUnit.Percentage);
                tableKhoaHoc.TableFormat.AutomaticallyResizeToFitContents = false;
                var tableBordersKhoaHoc = tableKhoaHoc.TableFormat.Borders;
                tableBordersKhoaHoc.SetBorders(MultipleBorderTypes.All, BorderStyle.None, Color.Empty, 0);
                tableKhoaHoc.Columns.Add(new TableColumn(40));
                tableKhoaHoc.Columns.Add(new TableColumn(60));
                TableRow tableRowKhoaHoc = new TableRow(document);
                tableKhoaHoc.Rows.Add(tableRowKhoaHoc);

                tableRowKhoaHoc.Cells.Add(new TableCell(document, new Paragraph(document, new Run(document, "Khóa học: ")
                {
                    CharacterFormat = new CharacterFormat()
                    {
                        Size = commonFontSize
                ...
                , new Run(document, "Số hiệu sinh viên: ")
                {
                    CharacterFormat = new CharacterFormat()
                    {
                        Size = commonFontSize
                ...
                , new Run(document, string.Format("{0} ", KyLuat))
                {
                    CharacterFormat = new CharacterFormat()
                    {
                        Bold = true,
                ...
                { ParagraphFormat = new ParagraphFormat { LineSpacing = 1.25 } }));
                tableRowKhoaHoc.Cells.Add(new TableCell(document, new Paragraph(document, new Run(document, "Thời gian khóa học: ")
                {
                    CharacterFormat = new CharacterFormat()
                    {
                        Size = commonFontSize
                    }
                }, new Run(document, string.Format("{0} (năm);", 5))
                {
                    CharacterFormat = new CharacterFormat()
                    {
                        Bold = true,
                ...
                }, new Run(document, string.Format("Chính quy"))
                {
                    CharacterFormat = new CharacterFormat()
                    {
                        Bold = true,
                ...
                section.Blocks.Add(tableKhoaHoc);
                #endregion
                #region Đề nghị
