<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="QuanLyDichVu_ThueNhaSinhVien.ascx.cs" Inherits="nuce.ad.ctsv.QuanLyDichVu_ThueNhaSinhVien" %>
...
<div class="row" style="padding-top: 5px;">
    ...
    <div class="col-sm-3">
        <div class="form-group">
            <select class="form-control" id="slChonNgay" onchange="QuanLyDichVu_ThueNhaSinhVien.changeNgayLayDuLieu(this);">
    ...
    <div class="col-sm-4" style="text-align: right">
        <button type="button" class="btn btn-success" 
                onclick="QuanLyDichVu_ThueNhaSinhVien.onClickConfirm(3)">Excel</button>
        <button type="button" class="btn btn-primary" 
                onclick="QuanLyDichVu_ThueNhaSinhVien.onClickConfirm(1)">In</button>
        <button type="button" class="btn btn-info" 
                onclick="QuanLyDichVu_ThueNhaSinhVien.onClickConfirm(2)">Chuyển trạng thái</button>
    </div>
</div>
<div>
            ...
            <tr>
                <th style="text-align: center; vertical-align: top;">
                    <input type="checkbox" class="checkbox-select"
                            id="select-all" onclick="QuanLyDichVu_ThueNhaSinhVien.onSelectAll()" />
                </th>
    ...
    <div class="modal-dialog">
        ...
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-default" id="btnCapNhat" onclick=" QuanLyDichVu_ThueNhaSinhVien.update();">Cập nhật</button>
            </div>
        </div>
    ...
    <div class="modal-dialog">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body" style="text-align: center;">
                ...
                <button type="button" class="btn btn-danger" id="btnXoa" onclick=" QuanLyDichVu_ThueNhaSinhVien.delete();">Thực hiện</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    var QuanLyDichVu_ThueNhaSinhVien = {
        ID: -1,
        Status: 1,
        Type: 7,
        Page: 1,
        ...
        afterConfirm: function(type) {
            ...
            const data = [];
            QuanLyDichVu_ThueNhaSinhVien.Data.forEach((item, index) => {
                if (QuanLyDichVu_ThueNhaSinhVien.selectedDataIndex.includes(index)) {
                    data.push({
                        ID: item.ID,
            ...
            switch(type) {
                    case 1:
                        ...
                        break;
                    case 2:
                        QuanLyDichVu_ThueNhaSinhVien.updateFourthStatus(jsonData);
                        break;
                    case 3:
        ...
        onClickConfirm: function(type) {
            if (QuanLyDichVu_ThueNhaSinhVien.selectedDataIndex.length > 0) {
                let content = '';
                switch(type) {
                    case 1:
                ...
                $('#btn-activate').click(function() { QuanLyDichVu_ThueNhaSinhVien.afterConfirm(type); });
        ...
        bindData: function () {
            $('#tbContent').html("");
            $('#ulPaging').html("");
            var SPage = (QuanLyDichVu_ThueNhaSinhVien.Page - 1) * QuanLyDichVu_ThueNhaSinhVien.RowCount + 1;
            var EPage = QuanLyDichVu_ThueNhaSinhVien.Page * QuanLyDichVu_ThueNhaSinhVien.RowCount;
            //Lấy dữ liệu
            $.getJSON(QuanLyDichVu_ThueNhaSinhVien.url + "ad_ctsv_qldv_xacnhan_get.ashx?Type=" + QuanLyDichVu_ThueNhaSinhVien.Type + "&&search=" + $('#myInput').val() + "&&CDay=" + $('#slChonNgay').val() + "&&SPage="+SPage+"&&EPage="+EPage, function (data) {
                //alert(data);
                $(`#select-all`).prop({checked: false, indeterminate: false});
                QuanLyDichVu_ThueNhaSinhVien.Data = data;
                QuanLyDichVu_ThueNhaSinhVien.SameStudentRequest = {};
                QuanLyDichVu_ThueNhaSinhVien.selectedDataIndex = [];
                var strHtml = "";
                var total = -1;
                $.each(data, function (i, item) {
                    total = item.Total;
                    if (item.StudentID in QuanLyDichVu_ThueNhaSinhVien.SameStudentRequest) {
                        QuanLyDichVu_ThueNhaSinhVien.SameStudentRequest[item.StudentID].push(i);
                    } else {
                        QuanLyDichVu_ThueNhaSinhVien.SameStudentRequest[item.StudentID] = [i];
                    }
                    ...
                    strHtml += `<input type="checkbox" id="${i}-select" 
                                    ...
                                    onclick="QuanLyDichVu_ThueNhaSinhVien.updateselectedDataIndex(${i}, 'select')">
                                </td>`;
                    ...
                    strHtml += "<td rowspan=\"2\" style=\"text-align: center; vertical-align: middle;\">";
                    strHtml += `<div class="vertical-group-btn"><button type="button" style="display: block; width: 100%" class="btn btn-primary"  onclick="return QuanLyDichVu_ThueNhaSinhVien.print(${item.ID});">In</button>`;
                    strHtml += `<button type="button" style="display: block; width: 100%" class="btn btn-info" data-toggle="modal" data-target="#myModal" onclick="QuanLyDichVu_ThueNhaSinhVien.fillData(${item.ID});">Chuyển trạng thái</button>`;
                    strHtml += `</td>`;
                    strHtml += `</div>`;
                    strHtml += "</tr>";
                    // row 2
                    ...
                    strHtml += `
                        ...
                        <input type="text" class="form-control" id="${item.ID}-HKTT_Pho" onkeyup="QuanLyDichVu_ThueNhaSinhVien.setValue(${i}, 'HKTT_Pho')" value="${item.HKTT_Pho || ''}"/>`;
                    strHtml += `</div>`;
                    strHtml += `<div class="col-sm-3 form-group edit-container">`;
                    strHtml += `
                        <label for="Email">Phường</label>
                        <input type="text" class="form-control" id="${item.ID}-HKTT_Phuong" onkeyup="QuanLyDichVu_ThueNhaSinhVien.setValue(${i}, 'HKTT_Phuong')" value="${item.HKTT_Phuong || ''}"/>`;
                    strHtml += `</div>`;
                    strHtml += `<div class="col-sm-3 form-group edit-container">`;
                    strHtml += `
                        <label for="Email">Quận/Huyện</label>
                        <input type="text" class="form-control" id="${item.ID}-HKTT_Quan" onkeyup="QuanLyDichVu_ThueNhaSinhVien.setValue(${i}, 'HKTT_Quan')" value="${item.HKTT_Quan || ''}"/>`;
                    strHtml += `</div>`;
                    strHtml += `<div class="col-sm-3 form-group edit-container">`;
                    strHtml += `
                        <label for="Email">Tỉnh/Thành phố</label>
                        <input type="text" class="form-control" id="${item.ID}-HKTT_Tinh" onkeyup="QuanLyDichVu_ThueNhaSinhVien.setValue(${i}, 'HKTT_Tinh')" value="${item.HKTT_Tinh || ''}"/>`;
                    strHtml += `</div>`;
                    strHtml += `</div>`;
                    strHtml += `<div class="row">`;
                ...
                //Phân trang
                if (total > QuanLyDichVu_ThueNhaSinhVien.RowCount) {
                    // <li><a href="javascript:alert('hi hi');">1</a></li>
                    //class="active"
                    var strHtmlPage = "";
                    var numPage = 1 + Math.floor((total - 1) / QuanLyDichVu_ThueNhaSinhVien.RowCount);
                  
                    for (var i = 1; i <= numPage; i++) {
                        if (i != QuanLyDichVu_ThueNhaSinhVien.Page)
                            strHtmlPage += "<li><a href=\"javascript:QuanLyDichVu_ThueNhaSinhVien.changePage(" + i + ");\">" + i + "</a></li>";
                        else {
        ...
        fillData: function(id) {
            //QuanLyDichVu_ThueNhaSinhVien.ID = id;
            this.ID = id;
        ...
        update: function () {
            ...
            $('.loader').show();
            $('#myModal').modal('hide')
            if (QuanLyDichVu_ThueNhaSinhVien.ID > 0) {
                $.getJSON(this.url + "ad_ctsv_qldv_xacnhan_changestatus.ashx?Type=" + QuanLyDichVu_ThueNhaSinhVien.Type + "&&ID=" + QuanLyDichVu_ThueNhaSinhVien.ID + "&&STATUS=" + strStatus + "&&PhanHoi=" + strPhanHoi + "&&NgayBatDau=" + strNgayBatDau + "&&NgayKetThuc=" + strNgayKetThuc, function (data) {
                    $('.loader').hide();
                    if (data == 1) {
                        $("#h_noidung_thongbao").html("Cập nhật thành công");
                        $('#myModalThongBao').modal();
                        QuanLyDichVu_ThueNhaSinhVien.bindData();
                    }
                    else {
                        $('#myModal').modal();
                        $("#edit_header_ThongBao").show();
        ...
        print: function (input) {
            ...
            $('#myModalConfirm').modal('hide');
            let data = {};
            const selected = QuanLyDichVu_ThueNhaSinhVien.Data.find(item => item.ID === input);
        ...
        changeNgayLayDuLieu: function (obj) {
            ...
            QuanLyDichVu_ThueNhaSinhVien.Page = 1;
            QuanLyDichVu_ThueNhaSinhVien.bindData();
        },
        changePage: function (page) {
            //alert($("#slChonNgay").val());
            QuanLyDichVu_ThueNhaSinhVien.Page = page;
            QuanLyDichVu_ThueNhaSinhVien.bindData();
        }
    };
    QuanLyDichVu_ThueNhaSinhVien.initData();
    QuanLyDichVu_ThueNhaSinhVien.bindData();
</script>
<script>
