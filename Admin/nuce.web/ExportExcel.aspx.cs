...
namespace TH.NUCE.Web
{
    public partial class ExportExcel : System.Web.UI.Page
    {
        protected override void OnInit(EventArgs e)
        {
            ...
            switch (iType)
            {
                ...
                case 6:
                    ...
                    break;
                case 7:
                    int ikithilophocid7 = int.Parse(Request.QueryString["kithilophocid"]);
                    ProcessKetQuaUngDungThi(ikithilophocid7);
                    break;
                case 8:
                    string search = Request.QueryString["search"] ?? "";
                    string danhmuc = Request.QueryString["danhmuc"] ?? "";
                    ProcessDanhSachTaiKhoanMatKhau(search, danhmuc);
                    break;
                case 9:
        ...
        #region export ket qua ung dung thi
        public void ProcessKetQuaUngDungThi(int KiThiLopHocID)
        {
            string constr = ConfigurationManager.ConnectionStrings["SiteSqlServer"].ConnectionString;
            using (SqlConnection con = new SqlConnection(constr))
            {
                using (SqlCommand cmd = new SqlCommand(string.Format(@"select b.Ma,b.Ho,b.Ten,a.MoTa,a.Diem from [dbo].[NuceThi_KiThi_LopHoc_SinhVien] a inner join [dbo].[NUCE_ThiChungChi_NguoiThi] b
on a.SinhVienID = b.ID where KiThi_LopHocID = {0}
order by ten, Ho, Ma
",KiThiLopHocID)))
                {
                    using (SqlDataAdapter sda = new SqlDataAdapter())
                    {
                        ...
                        {
                            sda.Fill(dt);
                            ExportToExcel(string.Format("ket_qua_ki_thi{0}", DateTime.Now.ToFileTimeUtc()), "ket_qua", dt);
                        }
                    }
                }
            }
        ...
        public void ProcessDanhSachTaiKhoanMatKhau(string search, string danhmuc)
        {
            string sql = string.Format(@"select a.Ma, a.MatKhau, a.Ho, a.Ten from [dbo].[NUCE_ThiChungChi_NguoiThi] a
               left join Nuce_ThiChungChi_DanhMuc b on a.DanhMucID=b.ID where (CMT like N'%{0}%' or a.Ma like N'%{0}%' 
                or a.Ten like N'%{0}%' or a.Ho like N'%{0}%') and  a.Status<>4 and (a.DanhMucID ={1} or {1}=-1) order by [UpdatedDate] desc", search, danhmuc);
            string constr = ConfigurationManager.ConnectionStrings["SiteSqlServer"].ConnectionString;
            using (SqlConnection con = new SqlConnection(constr))
            {
                using (SqlCommand cmd = new SqlCommand(sql))
                {
                    using (SqlDataAdapter sda = new SqlDataAdapter())
                    {
                        ...
                        {
                            sda.Fill(dt);
                            ExportToExcel(string.Format("DanhSachMatKhau_{0}", DateTime.Now.ToFileTimeUtc()), "DanhSachMatKhau", dt);
                        }
                    }
                }
            }
        }
        public void ProcessDanhSachLopThi(int KiThiLopHocID)
        {
            string constr = ConfigurationManager.ConnectionStrings["SiteSqlServer"].ConnectionString;
            using (SqlConnection con = new SqlConnection(constr))
            {
                using (SqlCommand cmd = new SqlCommand(string.Format(@"select b.Ma,b.Ho + ' ' + b.Ten as HoTen, CONVERT(VARCHAR(10), b.NgaySinh, 103) as NgaySinh, b.NoiSinh, b.Cmt, b.MatKhau 
                                            from [dbo].[NuceThi_KiThi_LopHoc_SinhVien] a 
                                            inner join [dbo].[NUCE_ThiChungChi_NguoiThi] b on a.SinhVienID = b.ID where KiThi_LopHocID = {0}
                                            order by ten, Ho, Ma
                                            ", KiThiLopHocID)))
                {
                    using (SqlDataAdapter sda = new SqlDataAdapter())
