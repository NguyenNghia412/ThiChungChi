...
where DichVuID = 1;", ID);
            ...
            if (dt1.Rows.Count > 0 && dt2.Rows.Count > 0)
            {
                ...
                Paragraph paragraphNoiDung = new Paragraph(document,
                    ...
                    new Run(document, "Kính gửi: Ban Quản lý vận hành Khu nhà ở sinh viên Pháp Vân."),
                    new SpecialCharacter(document, SpecialCharacterType.LineBreak),
                    new SpecialCharacter(document, SpecialCharacterType.Tab),
                ...
                rowT1CK.Cells.Add(new TableCell(document, new Paragraph(document,
                    new SpecialCharacter(document, SpecialCharacterType.LineBreak),
                    new SpecialCharacter(document, SpecialCharacterType.LineBreak),
                    new Run(document, "Xác nhận của cơ sở đào tạo")
                    {
                        CharacterFormat = new CharacterFormat()
                        {
                            Bold = true,
                    ...
                    new SpecialCharacter(document, SpecialCharacterType.LineBreak),
                    new Run(document, "(Ký, ghi rõ họ tên, chức vụ người ký, đóng dấu)")
                    {
                        CharacterFormat = new CharacterFormat()
                        {
                            Italic = true,
                ...
                , new SpecialCharacter(document, SpecialCharacterType.LineBreak)
                , new SpecialCharacter(document, SpecialCharacterType.LineBreak)
                , new Run(document, "Người viết đơn")
                {
                    CharacterFormat = new CharacterFormat()
                    {
                        Bold = true,
                ...
                , new SpecialCharacter(document, SpecialCharacterType.LineBreak)
                , new Run(document, "(Ký và ghi rõ họ tên)")
                {
                    CharacterFormat = new CharacterFormat()
                    {
                        Italic = true,
