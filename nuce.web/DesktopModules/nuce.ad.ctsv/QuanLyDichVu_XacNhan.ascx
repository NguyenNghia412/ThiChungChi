<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="QuanLyDichVu_XacNhan.ascx.cs" Inherits="nuce.ad.ctsv.QuanLyDichVu_XacNhan" %>
...
<div class="row" style="padding-top: 5px;">
    ...
    <div class="col-sm-3">
        <div class="form-group">
            <select class="form-control" id="slChonNgay" onchange="QuanLyDichVu_XacNhan.changeNgayLayDuLieu(this);">
    ...
    <div class="col-sm-4" style="text-align: right">
        <button type="button" class="btn btn-success" 
                onclick="QuanLyDichVu_XacNhan.onClickConfirm(3)">Excel</button>
        <button type="button" class="btn btn-primary" 
                onclick="QuanLyDichVu_XacNhan.onClickConfirm(1)">In</button>
        <button type="button" class="btn btn-info" 
                onclick="QuanLyDichVu_XacNhan.onClickConfirm(2)">Chuyển trạng thái</button>
    </div>
</div>
<div>
            ...
            <tr>
                <th style="text-align: center; vertical-align: top;">
                    <input type="checkbox" class="checkbox-select"
                            id="select-all" onclick="QuanLyDichVu_XacNhan.onSelectAll()" />
                </th>
                ...
                <th style="text-align: center; vertical-align: top;">Lý do</th>
    ...
    <div class="modal-dialog">
        ...
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-default" id="btnCapNhat" onclick=" QuanLyDichVu_XacNhan.update();">Cập nhật</button>
            </div>
        </div>
    ...
    <div class="modal-dialog">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body" style="text-align: center;">
                ...
                <button type="button" class="btn btn-danger" id="btnXoa" onclick=" QuanLyDichVu_XacNhan.delete();">Thực hiện</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    var QuanLyDichVu_XacNhan = {
        ...
        afterConfirm: function(type) {
            ...
            const data = [];
            QuanLyDichVu_XacNhan.Data.forEach((item, index) => {
                if (QuanLyDichVu_XacNhan.selectedDataIndex.includes(index)) {
                    data.push({
                        ID: item.ID,
            ...
            switch(type) {
                    case 1:
                        ...
                        break;
                    case 2:
                        QuanLyDichVu_XacNhan.updateFourthStatus(jsonData);
                        break;
                    case 3:
        ...
        onClickConfirm: function(type) {
            if (QuanLyDichVu_XacNhan.selectedDataIndex.length > 0) {
                let content = '';
                switch(type) {
                    case 1:
                ...
                $('#btn-activate').click(function() { QuanLyDichVu_XacNhan.afterConfirm(type); });
        ...
        bindData: function () {
            $('#tbContent').html("");
            $('#ulPaging').html("");
            var SPage = (QuanLyDichVu_XacNhan.Page - 1) * QuanLyDichVu_XacNhan.RowCount + 1;
            var EPage = QuanLyDichVu_XacNhan.Page * QuanLyDichVu_XacNhan.RowCount;
            //Lấy dữ liệu
            $.getJSON(QuanLyDichVu_XacNhan.url + "ad_ctsv_qldv_xacnhan_get.ashx?Type=" + QuanLyDichVu_XacNhan.Type + "&&search=" + $('#myInput').val() + "&&CDay=" + $('#slChonNgay').val() + "&&SPage="+SPage+"&&EPage="+EPage, function (data) {
                //alert(data);
                $(`#select-all`).prop({checked: false, indeterminate: false});
                QuanLyDichVu_XacNhan.Data = data;
                QuanLyDichVu_XacNhan.SameStudentRequest = {};
                QuanLyDichVu_XacNhan.selectedDataIndex = [];
                var strHtml = "";
                var total = -1;
                $.each(data, function (i, item) {
                    total = item.Total;
                    if (item.StudentID in QuanLyDichVu_XacNhan.SameStudentRequest) {
                        QuanLyDichVu_XacNhan.SameStudentRequest[item.StudentID].push(i);
                    } else {
                        QuanLyDichVu_XacNhan.SameStudentRequest[item.StudentID] = [i];
                    }
                    ...
                    strHtml += `<input type="checkbox" id="${i}-select" 
                                    ...
                                    onclick="QuanLyDichVu_XacNhan.updateselectedDataIndex(${i}, 'select')">
                                </td>`;
                    ...
                    strHtml += `<div style="font-weight: bold; max-width: 300px;">${item.LyDo}</div>`;
                    ...
                    strHtml += "<td rowspan=\"2\" style=\"text-align: center; vertical-align: middle;\">";
                    strHtml += `<div class="vertical-group-btn"><button type="button" style="display: block; width: 100%" class="btn btn-primary"  onclick="QuanLyDichVu_XacNhan.print(${item.ID});">In</button>`;
                    strHtml += `<button type="button" style="display: block; width: 100%" class="btn btn-info" data-toggle="modal" data-target="#myModal" onclick="QuanLyDichVu_XacNhan.fillData(${item.ID});">Chuyển trạng thái</button>`;
                    strHtml += `</td>`;
                    strHtml += `</div>`;
                    strHtml += "</tr>";
                    // row 2
                    ...
                    strHtml += `
                        ...
                        <input type="text" class="form-control" id="${item.ID}-HKTT_Pho" onkeyup="QuanLyDichVu_XacNhan.setValue(${i}, 'HKTT_Pho')" value="${item.HKTT_Pho || ''}"/>`;
                    strHtml += `</div>`;
                    strHtml += `<div class="col-sm-3 form-group edit-container">`;
                    strHtml += `
                        <label for="Email">Phường</label>
                        <input type="text" class="form-control" id="${item.ID}-HKTT_Phuong" onkeyup="QuanLyDichVu_XacNhan.setValue(${i}, 'HKTT_Phuong')" value="${item.HKTT_Phuong || ''}"/>`;
                    strHtml += `</div>`;
                    strHtml += `<div class="col-sm-3 form-group edit-container">`;
                    strHtml += `
                        <label for="Email">Quận/Huyện</label>
                        <input type="text" class="form-control" id="${item.ID}-HKTT_Quan" onkeyup="QuanLyDichVu_XacNhan.setValue(${i}, 'HKTT_Quan')" value="${item.HKTT_Quan || ''}"/>`;
                    strHtml += `</div>`;
                    strHtml += `<div class="col-sm-3 form-group edit-container">`;
                    strHtml += `
                        <label for="Email">Tỉnh/Thành phố</label>
                        <input type="text" class="form-control" id="${item.ID}-HKTT_Tinh" onkeyup="QuanLyDichVu_XacNhan.setValue(${i}, 'HKTT_Tinh')" value="${item.HKTT_Tinh || ''}"/>`;
                    strHtml += `</div>`;
                    strHtml += `</div>`;
                    strHtml += `<div class="row">`;
                ...
                //Phân trang
                if (total > QuanLyDichVu_XacNhan.RowCount) {
                    // <li><a href="javascript:alert('hi hi');">1</a></li>
                    //class="active"
                    var strHtmlPage = "";
                    var numPage = 1 + Math.floor((total - 1) / QuanLyDichVu_XacNhan.RowCount);
                  
                    for (var i = 1; i <= numPage; i++) {
                        if (i != QuanLyDichVu_XacNhan.Page)
                            strHtmlPage += "<li><a href=\"javascript:QuanLyDichVu_XacNhan.changePage(" + i + ");\">" + i + "</a></li>";
                        else {
        ...
        update: function () {
            ...
            $('.loader').show();
            $('#myModal').modal('hide')
            if (QuanLyDichVu_XacNhan.ID > 0) {
                $.getJSON(this.url + "ad_ctsv_qldv_xacnhan_changestatus.ashx?Type=" + QuanLyDichVu_XacNhan.Type + "&&ID=" + QuanLyDichVu_XacNhan.ID + "&&STATUS=" + strStatus + "&&PhanHoi=" + strPhanHoi + "&&NgayBatDau=" + strNgayBatDau + "&&NgayKetThuc=" + strNgayKetThuc, function (data) {
                    $('.loader').hide();
                    if (data == 1) {
        ...
        print: function (input) {
            ...
            $('#myModalConfirm').modal('hide');
            let data = {};
            const selected = QuanLyDichVu_XacNhan.Data.find(item => item.ID === input);
        ...
        changeNgayLayDuLieu: function (obj) {
            ...
            QuanLyDichVu_XacNhan.Page = 1;
            QuanLyDichVu_XacNhan.bindData();
        },
        changePage: function (page) {
            //alert($("#slChonNgay").val());
            QuanLyDichVu_XacNhan.Page = page;
            QuanLyDichVu_XacNhan.bindData();
        }
    };
    QuanLyDichVu_XacNhan.initData();
    QuanLyDichVu_XacNhan.bindData();
</script>
<script>
