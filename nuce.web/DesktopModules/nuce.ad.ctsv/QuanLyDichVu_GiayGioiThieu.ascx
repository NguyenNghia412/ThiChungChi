<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="QuanLyDichVu_GiayGioiThieu.ascx.cs" Inherits="nuce.ad.ctsv.QuanLyDichVu_GiayGioiThieu" %>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="form-group">
                    ...
                    <select class="form-control" id="TrangThai" onchange="QuanLyDichVu_GiayGioiThieu.changeTrangThai();">
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-default" id="btnCapNhat" onclick=" QuanLyDichVu_GiayGioiThieu.update();">Cập nhật</button>
            </div>
        </div>
    ...
    <div class="modal-dialog">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body" style="text-align: center;">
                ...
                <button type="button" class="btn btn-danger" id="btnXoa" onclick=" QuanLyDichVu_GiayGioiThieu.delete();">Thực hiện</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    var QuanLyDichVu_GiayGioiThieu = {
        ID: -1,
        Status: 1,
        Type:2,
        Data: [],
        ...
        bindData: function () {
            $('#tbContent').html("");
            $.getJSON(this.url + "ad_ctsv_qldv_xacnhan_get.ashx?Type=" + QuanLyDichVu_GiayGioiThieu.Type+ "&&search=" + $('#myInput').val(), function (data) {
                //alert(data);
                QuanLyDichVu_GiayGioiThieu.Data = data;
                var strHtml = "";
                $.each(data, function (i, item) {
                    ...
                    var strLydo = "<b>Kính gửi:</b> " + item.DonVi + "</br>";
                    strLydo += "<b>Đến gặp:</b> " + item.DenGap + "</br>";
                    strLydo += "<b>Về việc:</b> " + item.VeViec + "</br>";
                    switch (item.Status) {
                        case 2:
                    ...
                    if (item.Status < 4 && item.Status > 1) {
                        strHtml += "<td style=\"text-align: center; vertical-align: top;\"><button type=\"button\" class=\"btn btn-info\" data-toggle=\"modal\" data-target=\"#myModal\" onclick=\"QuanLyDichVu_GiayGioiThieu.fillData(" + item.ID + ");\">Chuyển trạng thái</button></td>";
                    }
                    else {
                        strHtml += "<td></td>";
                    }
                    strHtml += "<td style=\"text-align: center; vertical-align: top;\"><button type=\"button\" class=\"btn btn-primary\"  onclick=\"QuanLyDichVu_GiayGioiThieu.print('" + item.ID + "');\">In</button></td>";
                    strHtml += "</tr>";
        ...
        fillData: function (id) {
            QuanLyDichVu_GiayGioiThieu.ID = id;
            $("#edit_header").html("Chuyển trạng thái");
            $("#edit_header_ThongBao").hide();
            $.each(QuanLyDichVu_GiayGioiThieu.Data, function (i, item) {
                if (item.ID == id) {
                    ...
                    QuanLyDichVu_GiayGioiThieu.showHideFormTrangThai(item.Status);
                }
        ...
        {
            QuanLyDichVu_GiayGioiThieu.showHideFormTrangThai($("#TrangThai").val());
        },
        update: function () {
            // Kiem tra ma trang
            ...
            $('.loader').show();
            $('#myModal').modal('hide')
            if (QuanLyDichVu_GiayGioiThieu.ID > 0) {
                //Update
                $.getJSON(this.url + "ad_ctsv_qldv_xacnhan_changestatus.ashx?Type=" + QuanLyDichVu_GiayGioiThieu.Type + "&&ID=" + QuanLyDichVu_GiayGioiThieu.ID + "&&STATUS=" + strStatus + "&&PhanHoi=" + strPhanHoi + "&&NgayBatDau=" + strNgayBatDau + "&&NgayKetThuc=" + strNgayKetThuc, function (data) {
                    $('.loader').hide();
                    if (data == 1) {
    ...
    QuanLyDichVu_GiayGioiThieu.initData();
    QuanLyDichVu_GiayGioiThieu.bindData();
</script>
<script>
