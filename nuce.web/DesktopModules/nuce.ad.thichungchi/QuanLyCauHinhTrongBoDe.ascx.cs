...
{
    public partial class QuanLyCauHinhTrongBoDe : PortalModuleBase
    {
        ...
        {
            Response.Redirect(String.Format("/tabid/43/default.aspx?NguoiDung_MonHocid={0}&&bodeid={1}", NguoiDung_MonHocID, BoDeID));
        }
        private string getUrlReturn(int NguoiDung_MonHocID, int BoDeID)
        {
            return string.Format("/tabid/43/default.aspx?NguoiDung_MonHocid={0}&&bodeid={1}", NguoiDung_MonHocID, BoDeID);
        ...
        private void displayList(int NguoiDung_MonHocID, int BoDeID)
        {
            ...
            strTable += "<thead><tr>";
            strTable += string.Format("<th>{0}</div>", "STT");
            strTable += string.Format("<th>{0}</th>", "Tên");
            strTable += string.Format("<th>{0}</th>", "Mô tả");
            strTable += string.Format("<th>{0}</th>", "Chỉnh sửa");
            strTable += string.Format("<th>{0}</th>", "Xóa");
            strTable += string.Format("<th><a href='{0}&&EditType=insert'>Thêm mới</a></th>", getUrlReturn(NguoiDung_MonHocID, BoDeID));
            strTable += "</tr></thead>";
            ...
            for (int i = 0; i < tblTable.Rows.Count; i++)
            {
                ...
                strTable += " <tr style='color: red;font-weight:bold; font-size:12px;'>";
                strTable += string.Format("<td>{0}</td>", (i + 1));
                strTable += string.Format("<td>{0}</td>", tblTable.Rows[i]["Name"].ToString());
                strTable += string.Format("<td>{0}</div>", tblTable.Rows[i]["MoTa"].ToString());
                strTable += string.Format("<td><a href='{0}&&EditType=update&&bode_phanid={1}'>Sửa</a></td>", getUrlReturn(NguoiDung_MonHocID, BoDeID), BoDe_PhanID);
                strTable += string.Format("<td><a href='{0}&&EditType=delete&&bode_phanid={1}'>Xóa</a></td>", getUrlReturn(NguoiDung_MonHocID, BoDeID), BoDe_PhanID);
                strTable += string.Format("<td><a href='{0}&&EditType=insert'>Thêm mới</a></td>", getUrlReturn(NguoiDung_MonHocID, BoDeID));
                strTable += "<tr>";
                strTable += "<tr><td colspan='6'>";
            ...
            strTable += "<tr>";
            strTable += string.Format("<td colspan='6' style='height: 20px;text-align:right;font-weight:bold;'>Tổng số câu hỏi của cả bộ đề là: <span style='color:red;'>{0}</span></td></tr>", iTongSoCauHoi);
            strTable += "</tbody>";
            strTable += "</table>";
        ...
        protected void btnTaoDe_Click(object sender, EventArgs e)
        {
            ...
            {
                divAnnouce.InnerHtml = ex.ToString();
            }
        }
        ...
        {
            Response.Redirect(string.Format("/tabid/{0}/default.aspx?NguoiDung_MonHocid={1}", 42, txtNguoiDung_MonHocID.Text));
        }
