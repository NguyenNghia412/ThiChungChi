    ...
    public partial class QLNguoiThi : PortalModuleBase
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                {
                    if (int.TryParse(Request.QueryString["ID"].ToString(), out iID) && Request.QueryString["mode"] == "view")
                    {
                        sql = "select * from [dbo].[NUCE_ThiChungChi_NguoiThi] where ID=" + iID;
                        ...
                        if (dt.Rows.Count > 0)
                        {
                            ...
                            txtDanhMuc.InnerText = dt.Rows[0]["DanhMucID"].ToString();

                            txtMaThiSinh.InnerText = dt.Rows[0]["Ma"].ToString();
                            txtHoThiSinh.InnerText = dt.Rows[0]["Ho"].ToString();
                            txtTenThiSinh.InnerText = dt.Rows[0]["Ten"].ToString();
                            txtNgaySinhThiSinh.InnerText = DateTime.Parse(dt.Rows[0]["NgaySinh"].ToString()).ToString("dd/MM/yyyy");
                            txtNoiSinhThiSinh.InnerText = dt.Rows[0]["NoiSinh"].ToString();
                            txtGioiTinh.InnerText = dt.Rows[0]["GioiTinh"].ToString();
                            txtCmnd.InnerText = dt.Rows[0]["CMT"].ToString();
                            txtNgayCapCmnd.InnerText = DateTime.Parse(dt.Rows[0]["NgayCap"].ToString()).ToString("dd/MM/yyyy");
                            txtNoiCapCmnd.InnerText = dt.Rows[0]["NoiCap"].ToString();
                            txtSdt.InnerText = dt.Rows[0]["Mobile"].ToString();
                            txtMail.InnerText = dt.Rows[0]["Email"].ToString();
                            txtDiaChiThiSinh.InnerText = dt.Rows[0]["DiaChi"].ToString();
                            imgThiSinh.Src = dt.Rows[0]["Anh"].ToString();
                            string myScript = string.Format(@"<script>
            QuanLyNguoiThi.openModal('myModalThongTinThiSinh');
            </script>");
                            spScript.InnerHtml = myScript;
                        }
                    }
                    else if (int.TryParse(Request.QueryString["ID"].ToString(), out iID))
                    {
