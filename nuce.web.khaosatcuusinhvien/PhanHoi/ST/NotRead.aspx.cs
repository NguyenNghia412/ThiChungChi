...
{
    public partial class NotRead : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                {
                    DataSet ds = nuce.web.data.Nuce_DanhGiaGiangVien.getQA_QuestionNotReadByStudent(m_SinhVien.SinhVienID);
                    ...
                    if (dtSubject.Rows.Count > 0)
                    {
                        ...
                        for (int i = 0; i < dtSubject.Rows.Count; i++)
                        {
                            ...
                            for (int j = 0; j < dtType.Rows.Count; j++)
                            {
                                ...
                                if (dtType.Rows[j]["Template"].ToString().Equals(iTemplate.ToString()) && checkReadInType(ds.Tables[0], LecturerClassRoomID, Type))
                                {
                                    iCountType++;
                                    int iCountL2Temp = 0;
                    ...
                    {
                        infoSt.InnerText = string.Format("{0} {1} - {2}: Không có phản hồi nào là chưa đọc", m_SinhVien.Ho, m_SinhVien.Ten, m_SinhVien.MaSV);
                    }
                }
            }
        }
                                  ...
                                  <table class='table table-hover' style='width:100%;'>
                                    <thead>
                                        <tr style = 'vertical-align: top; text-align: center;'>
                                            ...
                                            <th style='text-align: center;vertical-align: top; width: 25%;'>Phản hồi của sinh viên</th>
                                            <th style='text-align: center;vertical-align: top; width: 35%;'>Hồi đáp của giảng viên</th>
        ...
        private bool checkReadInType(DataTable dt, int LecturerClassRoomID, int Type)
        {
            int iType, iLecturerClassRoomID;
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                iType = int.Parse(dt.Rows[i]["Type"].ToString());
                iLecturerClassRoomID = int.Parse(dt.Rows[i]["LecturerClassRoomID"].ToString());
